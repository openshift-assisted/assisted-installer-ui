...
**Before modifying a call site, ask: does this fix belong in the component itself?**
- If a component renders incorrectly, fix the component — do not inline its logic at the call site.
- Trace back to the source component file and fix it there.
- Example: if `<AssistedInstallerHeader />` renders wrong content, fix `AssistedInstallerHeader.tsx`, not the parent that renders it.
- Inlining component logic at a call site is never the right fix.
