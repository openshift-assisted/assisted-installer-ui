...
export const dualStackValidationSchema = (field: string, openshiftVersion?: string) =>
  Yup.array()
    ...
    .test(
      'dual-stack-ipv6',
      openshiftVersion && isMajorMinorVersionEqualOrGreater(openshiftVersion, '4.12')
        ? 'Second network has to be the opposite IP stack of the first network.'
        : 'Second network has to be IPv6 subnet.',
      (values?: { cidr: MachineNetwork['cidr'] }[]): boolean => {
        // For OCP versions > 4.11, allow IPv4 as secondary network if primary is IPv6
        if (openshiftVersion && isMajorMinorVersionEqualOrGreater(openshiftVersion, '4.12')) {
          const isFirstIPv6 = values?.[0].cidr && Address6.isValid(values[0].cidr);
          if (isFirstIPv6) {
            // If first is IPv6, second must be IPv4
            return !!values?.[1].cidr && Address4.isValid(values[1].cidr);
          }
        }
        // Default behavior: second network must be IPv6
        return !!values?.[1].cidr && Address6.isValid(values[1].cidr);
      },
    );
