...
export const getAICluster = ({
  ...
  const aiCluster: Cluster = {
    ...
    apiVips:
      agentClusterInstall?.status?.apiVIPs?.length || agentClusterInstall?.spec?.apiVIPs?.length
        ? (agentClusterInstall?.status?.apiVIPs || agentClusterInstall?.spec?.apiVIPs || []).map(
            (ip) => ({ ip }),
          )
        ...
        : [],
    ingressVips:
      agentClusterInstall?.status?.ingressVIPs?.length ||
      agentClusterInstall?.spec?.ingressVIPs?.length
        ? (
            agentClusterInstall?.status?.ingressVIPs ||
    ...
    clusterNetworks: agentClusterInstall?.spec?.networking?.clusterNetwork?.map((net) => ({
      cidr: net.cidr,
    ...
    machineNetworks: agentClusterInstall?.spec?.networking?.machineNetwork?.map((net) => ({
      cidr: net.cidr,
    })),
    monitoredOperators: [],
