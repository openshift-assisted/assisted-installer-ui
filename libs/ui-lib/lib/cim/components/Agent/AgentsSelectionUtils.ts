...
const getRequiredHosts = (
  ...
  // select hosts with requested role first
  const hostIds = agents
    .filter((agent) => getAgentRole(agent) === role)
    .map((agent) => agent.metadata?.uid)
    .slice(0, count);

  // if there aren't enough hosts with the requested role select from 'auto-assign' hosts
  // start from autoAssignOffset so previously selected 'auto-assign' hosts aren't selected again
