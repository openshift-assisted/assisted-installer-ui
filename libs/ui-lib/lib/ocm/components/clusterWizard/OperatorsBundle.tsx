...
import { getOperatorSpecs, OperatorSpec } from '../../../common/components/operators/operatorSpecs';

import './OperatorsBundle.css';

const getBundleLabel = (bundle: Bundle, opSpecs: { [key: string]: OperatorSpec }) => {
...
const BundleCard = ({
  ...
  const hasUnsupportedOperators = !!bundle.operators?.some((op) => {
    ...
    return !featureSupportLevelData.isFeatureSupported(operatorSpec.featureId);
  });
  ...
  return (
    <Tooltip content={disabledReason} hidden={!disabledReason}>
      ...
      >
        ...
        >
          <CardTitle>{getBundleLabel(bundle, opSpecs)}</CardTitle>
        </CardHeader>
        <CardBody isFilled>
          <Stack hasGutter>
            <StackItem isFilled>
