...
export const getAgentStatusFromConditions = (
  ...
  if (Installed?.status === 'True') {
    return ['installed', Installed.message];
  }
  ...
  if (Connected?.status === 'False') {
    return ['disconnected', Connected.message];
  }
  if (ReadyForInstallation?.status === 'True') {
    return ['known', ReadyForInstallation.message];
  }
  if (
  ...
  if (
    ReadyForInstallation?.status === 'False' &&
    ReadyForInstallation?.reason === 'AgentNotReady'
  ) {
