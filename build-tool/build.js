...
function getRollupCommands() {
  return rollupFormats.reduce((cmds, format) => {
    ...
    const env = [
      ...
      buildMinify ? `${isWindow ? 'set ' : ''}BUILD_MINIFY=${buildMinify}` : null,
      sizeSnapshot ? `${isWindow ? 'set ' : ''}BUILD_SIZE_SNAPSHOT=${sizeSnapshot}` : null,
      buildInput ? `${isWindow ? 'set ' : ''}BUILD_INPUT='${buildInput}'` : null,
    ]
      .filter(Boolean)
      .join(isWindow ? '&& ' : ' ');
